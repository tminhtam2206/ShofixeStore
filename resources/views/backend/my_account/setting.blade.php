    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item active">
                <a href="{{ route('backend.my_account.profile') }}"><i class="breadcrumb-icon fa fa-angle-left mr-2"></i>Tổng Quan</a>
            </li>
        </ol>
    </nav>
</header>
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            ...
            <div id="edit-password" class="card card-fluid" style="display: none;">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('backend.my_account.setting.change_password') }}" onsubmit="return checkFormPass()">
                        @csrf
                        <div class="form-group">
            ...
            <div id="edit-email" class="card card-fluid" style="display: none;">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('backend.my_account.setting.change_email') }}" onsubmit="return checkFormEmail()">
                        @csrf
                        <div class="form-group">
...
<script>
    ...
    $('#change-name').keyup(function() {
        ...
        if (name_val != '') {
            $.ajax({
                url: "{{ route('backend.my_account.setting.change_name') }}",
                data: {
                    name: $(this).val(),
                    _token: $('#token').val()
                },
    ...
    $('#change-introduce').keyup(function() {
        ...
        if (name_val != '') {
            $.ajax({
                url: "{{ route('backend.my_account.setting.change_introduce') }}",
                data: {
                    introduce: $(this).val(),
                    _token: $('#token').val()
                },
    ...
    $(document).ready(function() {
        ...
        $('.crop_image').click(function(event) {
            $image_crop.croppie('result', {
                ...
                $.ajax({
                    url: "{{ route('backend.my_account.setting.change_avatar') }}",
                    type: 'post',
                    data: {
                        "image": response,
                        _token: $('#token').val()
