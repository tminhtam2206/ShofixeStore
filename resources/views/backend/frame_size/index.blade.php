...
@section('title', 'Kích Thước Chung')
@section('content')
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <span></i>Kích Thước Chung</span>
                </li>
            </ol>
        </nav>
        <div class="d-md-flex align-items-md-start">
            <h1 class="page-title mr-sm-auto">KÍCH THƯỚC SẢN PHẨM</h1>
                        ...
                        <tbody>
                            ...
                            <tr id="tr-{{ $value->id }}">
                                ...
                                <td class="align-middle text-center">
                                    <button class="btn btn-sm btn-icon btn-primary mr-2" onclick="editName('{{ $value->size }}','{{ $value->id }}')">
                                        <i class="fa fa-pencil-alt"></i>
                                        <span class="sr-only">Edit</span>
                                    </button>
                                    <span class="btn btn-sm btn-icon btn-danger" onclick="DeleteCaterory('{{ $value->size }}','{{ $value->id }}')">
...
<form action="{{ route('backend.frame_size.add') }}" method="post" onsubmit="return checkAdd()">
    @csrf
    <div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-center" id="addModalLabel" style="width:100%;">THÊM KÍCH THƯỚC</h5>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="tenloai" name="size" autocomplete="off" maxlength="24" required />
                        <div class="invalid-feedback"><i class="fa fa-exclamation-circle fa-fw"></i><strong id="tenloai-error"></strong></div>
                    </div>
                </div>
                <div class="text-center pb-3">
                    <button type="submit" class="btn btn-primary"><i class="fal fa-save"></i> Lưu Kích Thước</button>
                </div>
            </div>
        </div>
    </div>
...
<form action="{{ route('backend.frame_size.edit') }}" method="post" onsubmit="return checkAdd()">
    @csrf
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="edit-tenloai" name="size" autocomplete="off" value="" maxlength="24" required />
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#tenloai').keyup(function() {
            ...
            if ($(this).val() != '') {
                $.ajax({
                    url: "{{ route('backend.frame_size.check') }}",
                    data: {size:key, _token:token},
                    type: 'post',
                    success: function(data_return) {
                        if (data_return == '0') {
                            ...
                            $('#tenloai-error').text('Kích thước [' + key + '] đã tồn tại!');
                            check_add = false;
                        }
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function(data_return) {
                        if (data_return == '0') {
                            ...
                            $('#edit-tenloai-error').text('Kích thước [' + key + '] đã tồn tại!');
                            check_add = false;
                        }
                    }
                });
    ...
    function DeleteCaterory(name, id_type) {
        $.confirm({
            ...
            content: 'Bạn có muốn <b>xóa</b> kích thước [<b>' + name + '</b>] không?' +
                '<br><small>Việc này sẽ làm những sản phẩm mới <b>không tìm thấy kích thước này</b></small>',
            type: 'red',
            buttons: {
                deleteUser: {
                    ...
                    action: function() {
                        $.ajax({
                            url: "{{ route('backend.frame_size.delete') }}",
